...
class Compiler(Visitor_Recursive):
    ...
    commandSymbols = { 'MATH_GREEK_SYMBOLS', 'MATH_BB_SYMBOLS', 'MATH_CAL_SYMBOLS', 'MATH_SYMBOLS', 'MATH_OPERATOR_A', 'MATH_OPERATOR_B' }
...
###################################################
    ...
    def operation(self, tree):
        ...
        if tree.children[0].data == 'operator':
            tree.result += tree.children[0].result + ' ' + tree.children[1].result
        
        elif tree.children[1].data == 'operator':
            tree.result += tree.children[0].result + ' ' + tree.children[1].result
    ...
    def operator(self, tree):
        tree.result = f'{self.operator_map[tree.children[0].value]}'
    ...
    def term(self, tree):
        ...
        for i, child in enumerate(tree.children):
            if isinstance(child, Tree):
                ...
                if i < len(tree.children) - 1 and child.type in self.commandSymbols:
                    tree.result += ' '        
